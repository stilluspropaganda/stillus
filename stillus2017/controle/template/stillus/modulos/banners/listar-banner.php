<?php if(isset($_SESSION['id_user']) && !empty($_SESSION['id_user'])){
	if(isset($_POST['acao']) && $_POST['acao']=='excluir'){
		...
		foreach($excluir as $x){
			$imgs = $sis->select("banners",NULL,NULL,"id='$x'");
			foreach($imgs as $img){
				unlink(p_UPLOADS.$img['foto']);
			}
			$del_item = $sis->delete("banners","id",$x);
		}
	}

	$itens = $sis->select("banners",NULL,NULL,NULL,"ordem ASC");
}else{
	exit;
}?>
<!-- BEGIN PAGE -->
	...
	<div class="container-fluid"> 
		...
		<div class="row-fluid">
			<div class="span12"> 
				...
				<h3 class="page-title">Banner <small>Lista</small> </h3>
				<ul class="breadcrumb">
									...
									<tbody>
										<?php foreach($itens as $item){?>
											<tr>
												...
												<td><a href="<?php echo $sis->url_base()?>controle/banners/editar-banner/<?php echo $item['id']?>" class="btn">Editar</a></td>
											</tr>
										<?php }?>
									</tbody>
								</table>
