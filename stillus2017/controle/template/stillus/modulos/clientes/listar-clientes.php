<?php if(isset($_SESSION['id_user']) && !empty($_SESSION['id_user'])){
	if(isset($_POST['acao']) && $_POST['acao']=='excluir'){
		...
		foreach($excluir as $x){
			$imgs = $sis->select("clientes",NULL,NULL,"id='$x'");
			foreach($imgs as $img){
				unlink(p_UPLOADS.$img['foto']);
			}
			$del_item = $sis->delete("clientes","id",$x);
		}
	}

	$itens = $sis->select("clientes",NULL,NULL,NULL,"ordem ASC");
}else{
	exit;
}?>
<!-- BEGIN PAGE -->
	...
	<div class="container-fluid"> 
		...
		<div class="row-fluid">
			<div class="span12"> 
				...
				<h3 class="page-title">Clientes <small>listar</small> </h3>
				<ul class="breadcrumb">
			...
			<div class="span12"> 
				...
				<div class="portlet box purple">
					<div class="portlet-title">
						<h4><i class="icon-list"></i>Cientes</h4>
					</div>
									...
									<tbody>
										<?php foreach($itens as $item){?>
											<tr>
												...
												<td><a href="<?php echo $sis->url_base()?>controle/clientes/editar-clientes/<?php echo $item['id']?>" class="btn">Editar</a></td>
											</tr>
										<?php }?>
									</tbody>
								</table>
