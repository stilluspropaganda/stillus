<?php if($url3){
	...
	echo'<script>window.location.href="'.$sis->url_base().'controle/clientes/listar-clientes";</script>';
	exit;
}?>
<!-- BEGIN PAGE -->
...
<?php if(isset($_POST['acao']) && $_POST['acao']=='cadastrar'){
	...
	$foto_titulo = 'clientes'.date('YmdHis').'_'.$sis->slug($nome).'_'.$sis->slug($foto['name']);
	...
	$update = $sis->update("clientes",$campos,"id",$id);
	if($update){
		$_POST = NULL;
...
$itens = $sis->select("clientes",NULL,NULL,"id='".$id."'");
	...
	<div class="container-fluid"> 
		...
		<div class="row-fluid">
			<div class="span12"> 
				...
				<h3 class="page-title"> Clientes <small>edição</small> </h3>
				<ul class="breadcrumb">
				...
				<div class="span12"> 
					...
					<div class="portlet box blue">
						<div class="portlet-title">
							<h4><i class="icon-reorder"></i>Editar clientes</h4>
						</div>
						<div class="portlet-body form"> 
							...
							<form action="" method="post" enctype="multipart/form-data" class="form-horizontal" id="commentForm">
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="texto" value="<?php if(isset($item['texto'])){echo $item['texto'];}?>" maxlength="250" class="span6 m-wrap" data-rule-required="true"  data-msg-required="Campo obrigatório." />
									</div>
								</div>
								<div class="control-group">
									<label class="control-label">Descrição:</label>
									<div class="controls">
										<input type="text" name="descricao" value="<?php if(isset($item['descricao'])){echo $item['descricao'];}?>" maxlength="255" class="span6 m-wrap" data-rule-required="true"  data-msg-required="Campo obrigatório." />
									</div>
								</div>
								<div class="control-group">
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										Trocar Foto: <label class="text-info">150x150pixels</label><input type="file" name="foto" class="span6 m-wrap" />
									</div>
								</div>							
								
								<div class="form-actions">
									<button type="submit" name="acao" value="cadastrar" class="btn blue">Salvar</button>
									<a href="<?php echo $sis->url_base()?>controle/clientes/listar-clientes" class="btn">Voltar</a>
								</div>
							</form>
							<!-- END FORM--> 
						</div>
